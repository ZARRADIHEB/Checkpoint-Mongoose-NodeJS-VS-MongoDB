...
const createManyPeople = async (peopleData) => {
  try {
...
//? Find People by favfood

const findPeopleByFood = async (favfood) => {
  //In case everything goes perfect
  try {
    const result = await Person.findOne({ favoriteFoods: favfood });
    if (result) {
...
//? find by id and add item to the array
